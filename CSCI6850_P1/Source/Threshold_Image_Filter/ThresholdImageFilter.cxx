...
//            argv[0]: ./ThresholdImageFilter
// INPUTS:    argv[1]: {jakob_rad_convention_stripped_with_cere.img}
// OUTPUTS:   {../Output_Images/threshold_image.img}
// ARGUMENTS: argv[2]: *Threshold provided by user*
//

// AUTHOR: Christian McDaniel
//
// This source code was modified from the ITK Example file
// {BinaryThresholdImageFilter.cxx}. It illustrates the use of the binary
// threshold image filter - which is used to transform an image into a
// binary image by changing the pixel values according to the rule
// illustrated in Figure~\ref{fig:BinaryThresholdTransferFunction} - in a
// modified fashion. Instead of a binary threshold, a single threshold
// intensity value is taken as an argument. In a binarizing fashion, all pixel
// values below this threshold are changed to 0, and all pixel values above
// this threshold are changed to 255. Additionally, the original code was
// modified so as to allow for a 3D input and output image type. The
...
int main( int argc, char * argv[] )
{
  ...
  // pixel types changed to accommodate 3D image
  // Software Guide : BeginCodeSnippet
  ...
  const InputPixelType Threshold = atoi( argv[2] );
  ...
  writer->SetFileName( "../Output_Images/threshold_image.img" );
  writer->Update();
